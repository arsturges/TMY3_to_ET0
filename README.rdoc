...
Generally, the model performs the following actions:
1. Create a hash (@hourly_data) to hold weather data
2. Begin looping through the 1020 TMY3 files
3. Collect characteristcs of the weather station (elevation, lat/long, time zone, etc)
4. Loop through the 8760 rows of a TMY3 file (representing each hour of year), and collect the relevant data (temperature, wind speed, etc). Calculate reference evapotranspiration for row (hour), and store the in an array called @station_array.
5. After looping through a particular TMY3 file, store its ET0 results in a bigger hash called @hourly_data.
6. When the last TMY3 file has been parsed and the last result stored in @hourly_data, flatten the array into national averages, by state and subregion (i.e. take the averages of the ET0 values for each hour)
7. Print the result to hourly.csv
8. Sum ET0 values for each month to get monthly ET0 values.
9. Print the result to monthly.csv.
...
==How to contribute
To contribute to this project, simply clone the project, make your changes, push to your local copy, and then send a pull request.
 $ git clone git://github.com/arsturges/TMY3_to_ET0.git 
 $ cd TMY3_to_ET0
 <make your changes to the code>
 $ git add .
 $ git commit -m "I fixed your terrible code."
 $ git push
Then send me a pull request from your fork on GitHub.
==License
